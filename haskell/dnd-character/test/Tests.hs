...
import DND (Character (..), ability, character, modifier)
...
specs = do
  ...
  where
    ...
    abilityWithinRange' character' (name, ability') =
      ...
      where
        msg = "The '" ++ name ++ "' ability is out of range"

    hitpointsAddUp :: Character -> Property
    hitpointsAddUp Character {..} =
      counterexample msg (hitpoints === 10 + modifier constitution)
      where
        msg = "The 'hitpoints' are not 10 + constitution modifier"
