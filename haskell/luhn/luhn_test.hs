...
creationTests =
  [ testCase
      ("Creating a valid number from " ++ show n)
      (expected @=? create n)
    | (expected, n) <- creations
  ]
...
checksumTests =
  [ testCase
      ("The checksum of " ++ show n)
      (expected @=? checksum n)
    | (n, expected) <- checksums
  ]
...
validityTests =
  [ testCase
      (show n ++ " should" ++ (if expected then "" else "n't") ++ " be valid")
      (expected @=? isValid n)
    | (n, expected) <- validityChecks
  ]
