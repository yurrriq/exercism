...
-type base_error() :: invalid_dst_base
                    | invalid_src_base.

-type digit_error() :: negative
                     | not_in_base.

-type error_tuple() :: {error, base_error() | digit_error()}.

-type valid_base () :: pos_integer().           % but actually `2..'


%%% ============================================================= [ Public API ]

%% @doc Convert a list of digits in `SourceBase' to `DestinationBase'.
-spec convert(InDigits, valid_base(), valid_base()) -> {ok, OutDigits} when
