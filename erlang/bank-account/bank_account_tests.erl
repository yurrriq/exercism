...
deposit_many_test() ->
    Bank_account = bank_account:create(),
    [
        erlang:spawn(fun() -> bank_account:deposit(Bank_account, X) end)
     || X <- lists:seq(1, 10)
    ],
...
withdraw_many_test() ->
    ...
    [
        erlang:spawn(fun() -> bank_account:withdraw(Bank_account, X) end)
     || X <- lists:seq(1, 10)
    ],
    First = bank_account:balance(Bank_account),
    timer:sleep(100),
...
charge_many_test() ->
    ...
    bank_account:deposit(Bank_account, 55),
    [
        erlang:spawn(fun() -> bank_account:charge(Bank_account, 10) end)
     || _X <- lists:seq(1, 10)
    ],
    First = bank_account:balance(Bank_account),
    timer:sleep(100),
