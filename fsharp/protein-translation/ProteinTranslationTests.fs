...
[<Fact>]
let ``Empty RNA sequence results in no proteins`` () = proteins "" |> should be Empty
...
[<Fact>]
let ``STOP codon RNA sequence 1`` () = proteins "UAA" |> should be Empty

[<Fact>]
let ``STOP codon RNA sequence 2`` () = proteins "UAG" |> should be Empty

[<Fact>]
let ``STOP codon RNA sequence 3`` () = proteins "UGA" |> should be Empty
...
[<Fact>]
let ``Translation stops if STOP codon at beginning of sequence`` () = proteins "UAGUGG" |> should be Empty
