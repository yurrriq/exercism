...
binary_to_multiple_decimal_test() ->
    ?assertMatch({ok, [4, 2]},
                 ?TESTED_MODULE:convert([1, 0, 1, 0, 1, 0], 2, 10)).


decimal_to_binary_test() ->
    ?assertMatch({ok, [1, 0, 1, 0, 1, 0]},
                 ?TESTED_MODULE:convert([4, 2], 10, 2)).
...
negative_digit_test() ->
    ?assertMatch({error, negative},
                 ?TESTED_MODULE:convert([1, -1, 1, 0, 1, 0], 2, 10)).


invalid_positiv_digit_test() ->
    ?assertMatch({error, not_in_base},
                 ?TESTED_MODULE:convert([1, 2, 1, 0, 1, 0], 2, 10)).
...
second_base_is_one_test() ->
    ?assertMatch({error, invalid_dst_base},
                 ?TESTED_MODULE:convert([1, 0, 1, 0, 1, 0], 2, 1)).
...
first_base_is_negative_test() ->
    ?assertMatch({error, invalid_src_base},
                 ?TESTED_MODULE:convert([1], -2, 10)).
